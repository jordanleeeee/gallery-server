...
const FavoritesPage = () => {
    ...
    useEffect(() => {
        if (!isClient || favorites.length === 0) return;
        ...
        const restore = sessionStorage.getItem("favoritesRestore");
        ...
        if (restore && previousScrollPosition && previousFavoriteCount && !restorationAttempted.current) {
            ...
            setTimeout(() => {
                ...
                sessionStorage.removeItem("favoritesRestore");
            }, 100);
        }
    ...
    }, [isClient, favorites, FAVORITES_PER_PAGE, displayedFavorites.length]);

    // Sync displayedFavorites with favorites when favorites change (e.g., when items are removed)
    useEffect(() => {
        if (!isClient || displayedFavorites.length === 0) return;
    ...
    }, [isClient, favorites, displayedFavorites, FAVORITES_PER_PAGE]);
    ...
    useEffect(() => {
        ...
        const onUnload = () => {
            ...
            sessionStorage.setItem("favoritesRestore", "true");
        };
    ...
    }, [router.events, isClient, displayedFavorites.length]);
